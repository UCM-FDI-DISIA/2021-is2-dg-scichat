...
public class Cell {
    ...
    public List<Cell> getNeighbours(){ return getNeighbours(1); }
    /**
     * Devuelve una lista inmutable de celdas, donde la primera es la
     * de la derecha, y el resto siguen en el orden de las agujas del 
     * reloj 
     * 
    ...
    public Cell getUpperRight(int times) throws OutOfBoundsException {
        if (row % 2 == 1) // Fila impar
            return new Cell(row - times, col + 1 - times%2, board);
        else  // Fila par
    ...
    Cell getByDirection(Direction dir) throws CellsNotLinedUpException, OutOfBoundsException { return getByDirection(dir, 1);}
    ...
    /**
     * Devuelve la distancia de una celda en la diagonal
     * de la actual a la actual.
     * 
    ...
    public int getDiagonalDistanceTo(Cell other) throws CellsNotLinedUpException {
    	if(this.isInSameDiagonalAs(other)) {
    		return Math.max(Math.abs(this.getCol() - other.getCol()), 
    						Math.abs(this.getRow() - other.getRow()));
    	}
    	else throw new CellsNotLinedUpException("Both cells must be in the same diagonal.");
    }
    
    public Cell getMiddleCellTowards(Cell other) throws CellsNotLinedUpException, InvalidOperationException, OutOfBoundsException {
    	Direction dir = this.getDirectionTowards(other);
    	int times = this.getDiagonalDistanceTo(other);
    	
    	if(times % 2 == 1) throw new InvalidOperationException("There is no middle cell.");
    	else {
     ...
     * @return 		verdadero si están en la misma diagonal 
     * 				falso si no están en la misma diagonal
     */   
