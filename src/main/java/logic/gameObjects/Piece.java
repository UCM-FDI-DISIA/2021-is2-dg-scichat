...
public class Piece {
	...
	if (targetPosition.equals(this.position)) {	//Comprobamos que la posición que queremos alcanzar  no es la de partida
	...
	if (!neighbours.contains(targetPosition)) {	//Si no encontramos la posición buscada entre los vecinos
	    ...
	    boolean[][] checkedCells = new boolean[Board.NUM_ROW][Board.NUM_COL];	//Creamos una matriz de booleanos que utilizaremos como mar
											//marcador para ver que piezas ya hemos estudiado
    ...
    /**
     * Método recursivo que nos devuelve true si se puede alcancar la posición o no. Para ello guardamos
     * en una cola las posiciones que podemos alcanzar a través de realizar saltos sobre otras fichas. Vamos
     * sacando los elementos de la cola y comprobando para cada una de esas posiciones los saltos que podemos realizar 
     * desde cada una de ellas. Guardamos esos nuevos saltos en la cola de posiciones para estudiarlas más adelante. Si no podemos
     * realizar nuevos saltos desde la posición actual, no podemos alcanzar la posición deseada desde la actual posicion, devolvemos falso.
     * El programa acaba cuando se haya un camino hasta la posición deseada o cuando se han estudiado todos y ninguno nos lleva a la misma. 
     * 
	...
	if (!checkedCells[positionToCheck.getRow()][positionToCheck.getCol()]) {	//Comprobamos que no la hemos estudiado ya
		...
		Cell newJump = positionToCheck.getCellJump(ady); //Vemos el posible salto que podemos obtener desde la posición actual y dado un vecio concreto
		if (newJump != null) {
		    if (newJump.equals(targetPosition))	//Comprobamos si el nuevo salto nos proporciona la posición deseada
			return true;
		    positionQueue.add(newJump);	//Añadimos la posición a la cola para estudiarla más adelante
		}
	    }

	    checkedCells[positionToCheck.getRow()][positionToCheck.getCol()] = true;	//Actualizamos la matriz de comprobaciones

	    return recursiveTryToMoveTo(targetPosition, checkedCells, positionQueue);	//Comprobamos las nuevas posiciones
	}
