...
public class Hard implements Strategy {
	...
	// In → Out ranks 0
	 ...
	 boolean fromOn = targets.contains(from),
		 toOn = targets.contains(to);
	 if(fromOn == toOn) return 0.5; // In → In or Out → Out
	 else if (!fromOn && toOn) return 1; // Out → In
	 else return 0; // In → Out
    }

    private double heuristic(Player player, Cell to, boolean jumpIsLimited, Board board, Piece piece) {
        Cell dest = board.getOppositeCornerCell(player.getSide()), 
        ...
        double symmetry = 0;
        GeoComp.Line line = new GeoComp.Line(new GeoComp.Point(from), new GeoComp.Point(to));
        for(Piece p : player.getPieces())
            symmetry += line.dist(new GeoComp.Point(p.getPosition()));
        // Max should be 68
        
        double rank = rankMove(piece.getPosition(), to, player);

        double dist_norm = (dist / MAX_DIST),
        	diam_norm = (diam / MAX_DIST),
        	sparse_norm = (sparse / MAX_AREA),
        	symmetry_norm = (symmetry / MAX_SYM),
        	rank_norm = (rank / MAX_RANK);
        System.out.println(
        	"Distance factor: "+dist_norm+
        	"\nDiameter factor: "+diam_norm+
        	"\nSparse factor: "+sparse_norm+
        	"\nSymmetry factor:"+symmetry_norm+
        	"\nRank factor:" + rank_norm
        );
        return (
            dist_norm * 
            IMP_DIST +
